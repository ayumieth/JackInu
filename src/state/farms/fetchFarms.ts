...
const fetchFarms = async () => {
  const data = await Promise.all(
    farmsConfig.filter(farm => farm.isSpecial === 'no').map(async (farmConfig) => {
      ...
      const totalWBNB = new BigNumber(tokenBalanceLP).div(new BigNumber(10).pow(tokenDecimals))
      const lpPriceInWBNB = totalWBNB.times(2).div(totalLP)

      const totalLiquidityWithoutPrice = lpPriceInWBNB.times(stakingLP)

      const amountJackTokens = new BigNumber(info.amountJackTokens._hex).div(new BigNumber(10).pow(quoteTokenDecimals))
      return {
        ...farmConfig,
        tokenAmount: tokenAmount.toJSON(),
        quoteTokenAmount: quoteTokenAmount.toJSON(),
        lpTotalInQuoteToken: lpTotalInQuoteToken.toJSON(),
        amountFastTokens: amountJackTokens.toJSON(),
